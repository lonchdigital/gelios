...
trait HandlesBriefBlocks
{
    ...
    protected function handleImageChangeForBriefBlocks($propertyName)
    {
        preg_match('/briefBlocks\.(\d+)\.newImage/', $propertyName, $matches);
        $index = $matches[1];
        $this->briefBlocks[$index]['temporaryImage'] = $this->briefBlocks[$index]['newImage']->temporaryUrl();
    }

    public function deleteImageBriefBlocks($index)
    {
        $this->briefBlocks[$index]['image'] = null;
        $this->briefBlocks[$index]['temporaryImage'] = null;
    }
